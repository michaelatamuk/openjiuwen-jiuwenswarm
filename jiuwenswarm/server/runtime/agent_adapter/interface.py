...
def _resolve_final_record_timestamp(
    *,
    ...
    if event_type in ("chat.llm_call_start", "chat.usage_metadata", "chat.llm_call_end"):
        # Anchor the whole LLM call (start / usage card / end) to the call's
        # start timestamp so the call events group together ahead of the
        # response text (chat.final), which is recorded at call completion.
