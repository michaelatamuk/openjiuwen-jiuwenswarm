...
export function useTrajectoryAnalysisCopy(): { zh: boolean; copy: Record<string, string> } {
  ...
  const copy = zh ? {
    ...
    rerun: '重新分析',
    running: '正在分析…',
    runningEstimate: '通常约需 1–2 分钟。期间可继续使用其他视图，结果会在这里展示。',
    stageReading: '读取会话记录…',
    stageAnalyzing: '扫描各轮次中的错误…',
    stageDiagnosing: '正在运行 LLM 诊断…',
    stageCompleted: '完成。',
    ...
    rerun: 'Analyze again',
    running: 'Analyzing…',
    runningEstimate: 'Usually about 1–2 minutes. You can keep using other views; we will show the result here.',
    stageReading: 'Reading the session record…',
    stageAnalyzing: 'Scanning turns for errors…',
    stageDiagnosing: 'Running LLM diagnosis…',
    stageCompleted: 'Done.',
