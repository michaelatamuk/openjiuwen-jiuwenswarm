...
```mermaid
flowchart TD
    I2(["input"]) --> M2["model"] --> D{"tool calls?"}
    D -->|no| O2(["final answer"])
    D -->|yes| T["run tools"] --> M2
```
...
flowchart TD
    ...
    T -->|no| F(["final answer"])
    T -->|"yes, under limits"| R["run tools"] --> M
    T -->|"limit hit"| S(["stop: rounds / time / budget / no-progress"])
```
...
```mermaid
flowchart TD
    I(["input"]) --> M["model turn"]
    M -->|"answer requests a todo_create tool call"| TC["ReAct loop runs TodoCreateTool"]
    TC --> P["TaskPlan / todos"]
    P --> W["work next task (reactive loop)"]
...
```mermaid
flowchart TD
    I(["complex task input"]) --> M["model turn"]
    M -->|"answer requests todo_create(list)"| TC["ReAct loop runs TodoCreateTool"]
    TC --> P["TaskPlan: ordered todos + depends_on"]
    P --> W["work next task (reactive loop)"]
    W --> U["update plan via todo_modify"]
    U --> Q{"plan complete?"}
    Q -->|no| W
...
flowchart TD
    ...
    subgraph REACT["Interleaved ReAct"]
...
```mermaid
flowchart TD
    I(["input"]) --> W["work the plan (reactive loop)"]
    W --> R["tool result differs from the plan"]
    R --> M{"how to adapt?"}
    M -->|"revise tasks"| T["todo_modify: add / cancel / reorder"]
    M -->|"new instruction"| S["push_steering"]
    M -->|"change mode"| P["enter/exit plan + approval"]
    T --> W
    S --> W
    P --> W
    W -->|"plan done"| A(["answer"])
```
...
```mermaid
flowchart TD
    C["tool call"] --> E{"failure or bad JSON?"}
    E -->|repairable args| FIX["repair JSON"] --> C
    E -->|"retryable + idempotent"| RETRY["retry (max 3)"] --> C
    E -->|"non-idempotent / exhausted"| ERR["structured error to the model"]
```
...
**Jiuwen:** Tool inputs are validated in `LocalFunction.invoke` before the function runs, via `SchemaUtils.validate_with_schema`. `StructuredOutputTool` forces schema-constrained tool arguments when the model lacks native structured output, and only force-finishes on success so failures reach the model. Workflow and agent-team schemas validate before use.
...
```mermaid
flowchart LR
    M["model output"] --> P["parse"] --> V{"schema valid?"}
    V -->|yes| A(["act"])
    V -->|no| R["reject / coerce → model retries"] --> M
```
...
```mermaid
flowchart TD
    L["tool loop"] --> G1{"max_iterations hit?"}
    G1 -->|yes| X1(["stop"])
    G1 -->|no| G2{"same tool + args N times?"}
    G2 -->|yes| X2(["compact warning or abort"])
    G2 -->|no| G3{"no-progress answer?"}
    G3 -->|yes| X3(["stop"])
    G3 -->|no| L
```
...
```mermaid
flowchart TD
    F["tool failed"] --> I{"ToolCard.idempotent?"}
    I -->|true| R(["auto-retry"])
    I -->|false| N(["do NOT retry — surface the error"])
```
...
```mermaid
flowchart TB
    subgraph ST["Short-term — SessionModelContext"]
        B["bounded message buffer"]
    end
    subgraph LT["Long-term — LongTermMemory"]
        V["variables · profile · semantic · episodic · summary"]
    end
    B -->|extract| LT
    LT -->|retrieve| B
```
...
```mermaid
flowchart TD
    T["turn"] --> A{"classifier: key info?"}
    A -->|no| D(["discard"])
    A -->|yes| E["extract candidates"] --> C{"vs existing: redundant / conflicting?"}
    C -->|redundant| X["delete old"]
    C -->|conflicting| X
    C -->|new| K(["keep"])
```
...
```mermaid
flowchart TD
    C["context grows"] --> B["buffer drops oldest"]
    B --> O["offload big messages / tool results"]
    O --> M["micro-compact old tool results"]
    M --> F["full / round compact at token threshold"]
    F --> P["cap long-term promotions"]
```
...
```mermaid
flowchart LR
    CTX["old context"] --> TH{"over token threshold?"}
    TH -->|no| KEEP(["keep verbatim"])
    TH -->|yes| SUM["summarize + boundary marker"]
...
```mermaid
flowchart TB
    L1["iteration / round / time caps"] --> L2["BudgetNoticeRail: wind-down prompt"]
    L2 --> L3["session cost cap (enforced, pre-flight + mid-stream)"]
    L3 --> L4["team token ledger: force-finish on exhaustion"]
```
...
```mermaid
flowchart TD
    AC["TokenBudgetEvaluator + LoopCoordinator.add_token_usage"] --> G{"production caller?"}
    G -->|none| INERT(["wired but inert"])
    SC["session cost cap"] --> ENF(["actually enforced"])
```
...
```mermaid
flowchart TD
    A["tool / action"] --> P["permission engine"]
    P --> TP["tool policy (tiered)"]
    P --> FG["file guard"]
    P --> NG["net guard"]
    TP --> M{"merge by strictest"}
    FG --> M
    NG --> M
    M -->|allow| X(["run"])
    M -->|ask| H(["human approval"])
    M -->|deny| B(["blocked"])
```
...
```mermaid
flowchart TD
    T["tool result"] --> MSG["ToolMessage → model context"]
    MSG --> GAP["no untrusted-data framing or sanitization"]
    GAP --> RISK(["prompt-injection risk"])
    H["opt-in heuristic scan"] -.->|only partial guard| MSG
    S["sanitize.py helpers"] -.->|no production callers| MSG
```
...
```mermaid
flowchart TD
    C["code action"] --> P{"permissions.enabled?"}
    P -->|no| A["runs (no rail)"]
    P -->|yes| D{"ALLOW / ASK / DENY"}
    D -->|allow| A
    D -->|ask| H["human approval → resume"] --> A
    D -->|deny| X(["blocked"])
    A --> S{"sandbox mode?"}
    S -->|local| LOCAL(["policy-only, not OS-isolated"])
    S -->|jiuwenbox| ISO(["Linux bwrap / Landlock / namespaces"])
```
