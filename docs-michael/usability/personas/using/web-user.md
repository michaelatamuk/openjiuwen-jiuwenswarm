...
*The person who runs JiuwenSwarm on their machine and uses the full Web UI / desktop app for their own tasks.*

This is the most common way JiuwenSwarm is used: someone installs it and talks to it from the
Web UI (`localhost:5173`) or desktop app, with full access — chat, settings, skills, trajectory.
It is also usually a technical person who self-hosts, so in that case they are also the
Instance Admin (see the note below).

If you reach JiuwenSwarm from a chat app instead (a Feishu/Telegram/WeChat group bot) with no
Web UI, you are a **Chat User**, not this persona.

**Note on overlap with the Instance Admin:** in the dominant pattern a developer or technical
person self-hosts and is both the Web User and the Instance Admin; the two collapse into one.
The split matters only when an instance is run for other people.
