...
**Two meaningfully different variants of the end-user exist:**
...
- **Web UI user** — opens `localhost:5173` directly, has full access to the chat
  ...
  (see note on the End-User / Instance Admin overlap below).
- **IM channel user** — encounters jiuwenswarm through a Feishu group, Telegram
  ...
  may not know they are talking to jiuwenswarm. This is a mode of access, not a
  separate persona, and is covered here alongside the Web UI user.
...
**Note on the End-User / Instance Admin overlap:** In the dominant deployment pattern — a
developer or technical individual who self-hosts jiuwenswarm — the installer and the
user are the same person. The End-User and Instance Admin collapse into one. The split is
meaningful only when jiuwenswarm is deployed as a shared IM bot: one person configures
it, others use it through a channel without touching the config.
